Libretto.Animation = class {
  ...
  execKeyframe(afterTime) {
    ...
    if (keyframe.delay() === null) {
      ...
      delay = Libretto.Animation.timeToMillisecond(keyframe.delay()) + afterTime + 'ms';
    }
    ...
    if (!this.hasNextKeyframe()) { return; }
    let next_key = this.keyframes[this.index];
