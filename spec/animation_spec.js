...
describe('Test of animation.coffee', function() {
  ...
  it('pops the next key frame', function() {
    ...
    sec0.innerHTML += '<animation>' +
                      ' <keyframe target="#div_id"' +
                      '           duration="500ms" delay="500ms"' +
                      '           color="red"></keyframe>' +
                      '</animation>';
    let anime = new Animation(page0);
    anime.nextKeyframe();
  ...
  it('converts the duratin text to millisec', function() {
    expect(Animation.timeToMillisecond("200ms")).to.equal(200);
    expect(Animation.timeToMillisecond("5s")).to.equal(5000);
    expect(Animation.timeToMillisecond("1.4s")).to.equal(1400);
    expect(Animation.timeToMillisecond("abc1.4s")).to.be.null;
    expect(Animation.timeToMillisecond("x")).to.be.null;
    expect(Animation.timeToMillisecond("ms")).to.be.null;
    expect(Animation.timeToMillisecond("")).to.be.null;
  });
});
